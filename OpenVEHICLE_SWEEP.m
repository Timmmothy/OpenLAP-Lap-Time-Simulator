%% HUD
[folder_status,folder_msg] = mkdir('SWEEPS_VEHICLE/'+SWEEP_FOLDER) ;
vehname = "SWEEPS_VEHICLE/"+SWEEP_FOLDER+"/OpenVEHICLE_"+name+"_"+type+"_"+SWEEP_PARAMS_FILE_EXTENSION ;
delete(vehname+".log") ;
diary(vehname+".log") ;
disp([...
...
Xpos = floor((SS(3)-W)/2) ;
Ypos = floor((SS(4)-H)/2) ;
f = figure('Name','Vehicle Model','Position',[Xpos,Ypos,W,H],'Visible','off') ;
sgtitle(name)
