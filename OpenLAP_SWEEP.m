...
%% vehicle file
vehiclefile = "SWEEPS_VEHICLE/"+SWEEP_FOLDER+"/OpenVEHICLE_"+name+"_"+type+"_"+SWEEP_PARAMS_FILE_EXTENSION+'.mat' ;
...
simname = "SWEEPS_LAP/"+SWEEP_FOLDER+"/OpenLAP_"+char(veh.name)+"_"+tr.info.name+date_time+"_"+SWEEP_PARAMS_FILE_EXTENSION ;
logfile = simname+".log" ;

%% HUD

[folder_status,folder_msg] = mkdir('SWEEPS_LAP/'+SWEEP_FOLDER) ;
delete(simname+".log") ;
...
CURRENT_LAPTIME = sim.laptime.data ;
global CURRENT_SIM
CURRENT_SIM = sim ;
...
Xpos = floor((SS(3)-W)/2) ;
Ypos = floor((SS(4)-H)/2) ;
f = figure('Name','OpenLAP Simulation Results','Position',[Xpos,Ypos,W,H],'Visible','off') ;
